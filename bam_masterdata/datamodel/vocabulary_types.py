# Auto-generated from Excel (Vocabulary Type rows)
# NOTE: German translations for ENTRY text are not available in the sheet;
#       categories and common options (Yes/No, Not applicable, Other...) were translated via a small mapping.
# Source: 2025-12_Data-Schema_Creep_v2.0_openBIS_filtered_vocabulary_type_working.xlsx

# from your_module import VocabularyType, VocabularyTypeDef, VocabularyTerm

################################################################################################################################

# The following terms need to be copied to the bam-masterdata file on BAM-Research GitHub to define the vocabulary types.
# Accordingly, the file can be found [here](https://github.com/BAMresearch/bam-masterdata/blob/main/bam_masterdata/datamodel/vocabulary_types.py).
...
class MultistageAnnealing(VocabularyType):
    defs = VocabularyTypeDef(
        code="CREEP_TEST_MULTISTAGE_ANNEALING",
        description="""Multistage Annealing - Was a multi-stage annealing process performed? (Metadata, Material history and condition, Heat treatment) // Mehrstufiger Annealing-Prozess - Wurde ein mehrstufiger Annealing-Prozess durchgeführt? (Metadaten, Materialhistorie und Zustand, Wärmebehandlung)""",
    )
...
class MultistageAgeing(VocabularyType):
    defs = VocabularyTypeDef(
        code="CREEP_TEST_MULTISTAGE_AGEING",
        description="""Multistage Ageing - Did the material undergo a multistage ageing process? (Metadata, Material history and condition, Heat treatment) // Mehrstufiger Alterungsprozess - Unterlag das Material einem mehrstufigen Alterungsprozess? (Metadaten, Materialhistorie und Zustand, Wärmebehandlung)""",
    )
...
class CreepTestNumberOfThermocouples(VocabularyType):
    defs = VocabularyTypeDef(
        ...
        description="""Number of Thermocouples (Metadata, Measuring and test equipment, Temperature-measuring system, Temperature sensor) // Anzahl der Thermoelemente (Metadaten, Mess- und Prüfmittel, Temperaturmesssystem, Temperatursensor)""",
    )

    term_1 = VocabularyTerm(
        code="1",
        label="1",
        description="""1 // 1""",
    )

    term_2 = VocabularyTerm(
        code="2",
        label="2",
        description="""2 // 2""",
    )

    term_3 = VocabularyTerm(
        code="3",
        label="3",
        description="""3 // 3""",
    )
...
class CreepTestMeasurementSetUp(VocabularyType):
    defs = VocabularyTypeDef(
        ...
        description="""Measurement Set-up - Measurement one-sided or two-sided? (Metadata, Measuring and test equipment, Extension values, Contacting extensometer) // Messaufbau - Einseitige oder beidseitige Messung? (Metadaten, Mess- und Prüfmittel, Dehnungswerte, kontaktierendes Extensometer)""",
    )
...
class MeasuringEquipmentCalibrationResult(VocabularyType):
    defs = VocabularyTypeDef(
        code="CREEP_TEST_MEASURING_EQUIPMENT_CALIBRATION_RESULT",
        description="""Calibration Result (Metadata, Measuring and test equipment, Elongation values and cross-sectional dimensions) // Calibration Result (Metadaten, Mess- und Prüfmittel, Dehnungswerte und Querschnittsabmessungen)""",
    )

    meets_calibration_requirements = VocabularyTerm(
        code="MEETS_CALIBRATION_REQUIREMENTS",
        label="Meets calibration requirements",
        description="""Meets calibration requirements // Erfüllt die Kalibrierungsanforderungen""",
    )

    does_not_meet_calibration_requirements = VocabularyTerm(
        code="DOES_NOT_MEET_CALIBRATION_REQUIREMENTS",
        label="Does not meet calibration requirements",
        description="""Does not meet calibration requirements // Erfüllt nicht die Kalibrierungsanforderungen""",
    )
...
TestStandard = CreepTestStandard
